...
│   ├── SessionStore.kt          # DataStore-backed JSON of the playback session (playlist + index + position + playSequence + audioOnly); restored on fresh process, cleared on task removal (queue stays in-memory until next play)
│   └── LibraryPreferences.kt    # DataStore-backed ViewMode + GroupBy + restart-on-previous + playSequence prefs
├── browse/
...
Process death reclaims the native mpv handle and all in-memory `PlayerState`. To survive this, `PlayerController` (on `init`) restores the last saved session from `SessionStore` (a DataStore-backed JSON of playlist + index + position + playSequence + audioOnly) once the library has been scanned, leaving mpv **paused at the saved position**. Tree-URI tracks re-resolve against `state.library` (by `mediaId`/`(parentUri, fileName)`); non-tree-URI tracks fall back to a `ContentResolver.openInputStream` existence probe (like `PlaylistController`); if the saved current track is unavailable the session is cleared and no restore happens. A 5 s heartbeat while playing plus a 500 ms-debounced `saveSessionSoon()` on every meaningful change (track load, skip, sequence change, audio-only toggle, pause) keep the saved session fresh. `stopPlayback()` (called from `PlayerService.onTaskRemoved`/`onDestroy`) releases mpv and clears the saved session, but **preserves the queue in `_state`** (the playlist, current index, and position are kept) so reopening the app after swiping from recents shows the mini player with the last track paused at its position; tapping play (or the lockscreen/notification play button, which routes through `togglePlayPause`/`setPlaying`) calls `resumeRememberedPlayback()`, which restarts the foreground service + mpv, reloads the current track at the saved position, and unpauses. The controller scope stays alive, so this works even when the process survives the swipe; on a genuine fresh process, `restoreSessionIfPresent()` repopulates `_state` from `SessionStore` (paused) instead. `Player.stop()` only zeroes `isPlaying` and the native handle — it no longer drops the playlist/currentIndex/position, so the remembered queue survives a swipe in the same process.

- **`PlayerController`** (`player/PlayerController.kt`) — owns `Player`, `FolderRepository`, `ArtCache`, `FileCopyCache`, `MetadataCache`, `SessionStore`. Exposes `state: StateFlow<PlayerState>` and the methods UI calls (`selectTrack`, `playFromLibrary`, `playAdhoc`, `next`, `previous`, `togglePlayPause`, `seekTo`, `setAudioOnly`, `setPlaylistUserOverride`, `cyclePlaySequence`, `setPlaySequence`, `resumeRememberedPlayback`, folder grant/revoke/refresh). `ensureServiceStarted()` does `startForegroundService(PlayerService)`. Playback works without any UI attached (background). On `init` it tries to restore the last saved session from `SessionStore` (paused at the saved position) and collects `LibraryPreferences.playSequence()` to keep `_state.playSequence` in sync with the persisted preference (`restoredOnce` gates the collector so it won't clobber a session-restored mode while a restore is pending). A periodic heartbeat plus a debounced `saveSessionSoon()` persist the playlist/index/position/playSequence/audioOnly on every meaningful change. `stopPlayback()` releases mpv, clears the saved session, resets `serviceStarted`, and — if there was a live queue — keeps it in `_state` (paused, no art/video) and sets `stoppedWithRememberedQueue = true` so the mini player keeps showing the last track; otherwise it resets `_state` to an empty `PlayerState` (preserving `playSequence`). `release()` additionally cancels the scope (only for the genuine Application teardown path). `togglePlayPause`/`setPlaying(true)`/`seekTo` while `stoppedWithRememberedQueue` is set route through `resumeRememberedPlayback()` (restart service + mpv + reload at saved position + unpause); the fresh-load paths (`playFromLibrary`/`playDirectory`/`playAdhoc`/`selectTrack`) clear the flag.
- **`PlayerService`** (`player/PlayerService.kt`) — `MediaSessionService`. `onCreate` starts mpv, builds `MpvPlayerAdapter`, creates and registers the `MediaSession`, collects `controller.state` on Main, and calls `adapter.refresh()` (= `invalidateState()`). The initial foreground notification is `Notification.MediaStyle`-backed with the platform session token; Media3's default provider subsequently publishes the metadata-driven notification. `onTaskRemoved` calls `controller.stopPlayback()` (releases mpv, clears the saved session, resets `serviceStarted` — but keeps the `Application`-owned controller alive and the remembered queue in `_state`), removes the foreground notification, and stops the service so swiping the app from recents stops playback while the last track stays available in the mini player for a one-tap resume. `onDestroy` likewise calls `stopPlayback()` and releases the media3 session/adapter.
...
`PlaySequence` (`player/PlaySequence.kt`) is the enum stored in `PlayerState.playSequence` (session-only, defaults to `SEQUENCE`). Pure helpers `indexAfterTrackEnd(mode, size, current)` and `indexForSkip(mode, size, current, forward)` compute the next index for end-of-track auto-advance and for the manual prev/next buttons respectively — both are unit-tested in `PlaySequenceTest`. `PlayerController.cyclePlaySequence()`/`setPlaySequence(mode)` mutate the mode; `setPlaySequence` additionally persists the choice to `LibraryPreferences` (a DataStore-backed `play_sequence` key) so it survives reboots and task removal, and a collector in `PlayerController.init` re-applies the persisted mode (gated by `restoredOnce` so it won't clobber a session-restored mode).
...
4. **Settings persistence** — store mpv option choices, recents, `audioOnly`, view-mode/group-by/filter, enriched metadata cache, etc. in `DataStore` so they survive reboots. `LibraryPreferences`, `MetadataCache`, `BookmarkRepository`, and `SessionStore` (playback session) already use DataStore; extend to the rest (mpv options, recents).
