...
│   └── PlaylistController.kt    # app-singleton: playlists StateFlow + resolved StateFlow (entries matched against library by fileName+parentUri, with ContentResolver existence fallback for non-tree URIs); enriches added items via MediaMetadataRetriever; create/add/remove/toggleFavorite/isFavorite
└── ui/
